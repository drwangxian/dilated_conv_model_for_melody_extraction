...
adc04: 0.7252167410718732
mirex05: 0.7975679203844672
mdb: 0.7409380463247187
mir1k: 0.6136242206920905
rwc: 0.6642214564939166
...
class Config:
    ...
    def __init__(self):
        ...
        self.train_or_inference = Namespace(
            inference=os.path.join('..', '..', 'checkpoints/vocal/gao', 'd0-6'),
            from_ckpt=None,
            ckpt_prefix=None
        )
        self.tb_dir = 'tb_inf'
        ...
        if self.train_or_inference.inference is None:
            ...
            spec = tf.random.normal([1, self.snippet_len, 256, 3])
...
class AcousticModel:
    ...
    @tf.function(input_signature=[
        tf.TensorSpec([None]),
        tf.TensorSpec([None, 257])
    ], autograph=False)
    def loss_tf_fn(self, ref_notes, logits):
        # smoothing

        note_range = TFDataset.note_range
        assert len(note_range) == 256 and note_range[0] > 0
...
class TFDataset:
    ...
    def gen_label_fn(track_id):
        ...
        num_frames_16ms = ((num_frames - 1) * 160 + 440) // 441 + 1
        times_new = np.arange(num_frames_16ms) * 0.016
        assert times_new[-1] > times_labels[-1, 0]
        vocal_freqs_160ms, _ = mir_eval.melody.resample_melody_series(
            times=times_labels[:, 0],
            frequencies=vocal_freqs,
            voicing=vocal_freqs > .1,
            times_new=times_new
        )
        TFDataset.validity_check_of_ref_freqs_fn(vocal_freqs_160ms)
        notes = TFDataset.hz_to_midi_fn(vocal_freqs_160ms)
...
class TFDatasetForAdc04(TFDataset):
    ...
    def gen_label_fn(track_id):
        ...
        times_256_44100 = np.arange(num_frames) * (256. / 44100.)
        num_frames_256_16000 = ((num_frames - 1) * 160 + 440) // 441 + 1
        times_256_16000 = np.arange(num_frames_256_16000) * 0.016
        assert times_256_16000[-1] >= times_256_44100[-1]

        freqs_256_16000, _ = mir_eval.melody.resample_melody_series(
            times=times_256_44100,
            frequencies=freqs,
            voicing=freqs > .1,
            times_new=times_256_16000
        )
        TFDataset.validity_check_of_ref_freqs_fn(freqs_256_16000)
        notes = TFDataset.hz_to_midi_fn(freqs_256_16000)
...
class TFDatasetForMirex05(TFDataset):
    ...
    def gen_label_fn(track_id):
        ...
        times_10ms = np.arange(num_frames) * 0.010
        num_frames_16ms = ((num_frames - 1) * 5 + 7) // 8 + 1
        times_16ms = np.arange(num_frames_16ms) * 0.016
        assert times_16ms[-1] >= times_10ms[-1]
        freqs_16ms, _ = mir_eval.melody.resample_melody_series(
            times=times_10ms,
            frequencies=freqs_441,
            voicing=freqs_441 > .1,
            times_new=times_16ms
        )
        TFDataset.validity_check_of_ref_freqs_fn(freqs_16ms)
        notes = TFDataset.hz_to_midi_fn(freqs_16ms)
...
class TFDatasetForMir1k(TFDataset):
    ...
    def gen_label_fn(track_id):
        ...
        num_frames_16ms = ((num_frames - 1) * 5 + 3) // 4 + 1
        times_16ms = np.arange(num_frames_16ms) * 0.016
        assert times_16ms[-1] >= times_20ms[-1]
        pitches_16ms, _ = mir_eval.melody.resample_melody_series(
            times=times_20ms,
            frequencies=pitches,
            voicing=pitches > .1,
            times_new=times_16ms
        ...
        result = dict(notes=pitches_16ms, original=dict(times=times_20ms, freqs=freqs_20ms))
...
class TFDatasetForRWC(TFDataset):
    ...
    def gen_label_fn(self, track_id):
        ...
        num_frames_16ms = 1 + ((num_frames_10ms - 1) * 5 + 7) // 8
        times_10ms = np.arange(num_frames_10ms) * 0.01
        times_16ms = np.arange(num_frames_16ms) * 0.016
        assert times_16ms[-1] >= times_10ms[-1]
        TFDataset.validity_check_of_ref_freqs_fn(freqs_10ms)
        voicing_10ms = freqs_10ms > .1
        freqs_16ms, _ = mir_eval.melody.resample_melody_series(
            times=times_10ms,
            frequencies=freqs_10ms,
            voicing=voicing_10ms,
            times_new=times_16ms
        )
        TFDataset.validity_check_of_ref_freqs_fn(freqs_16ms)
        notes = TFDataset.hz_to_midi_fn(freqs_16ms)
...
class MetricsInference(MetricsBase):
    ...
    def mir_eval_oa_fn(self, rec_idx, est_notes):
        ...
        est_times = np.arange(num_frames) * 0.016
    ...
    def __init__(self, config, name):
        ...
        if inferencing:
            if 'test' not in self.name:
                ...
                self.tf_dataset = TFDatasetForAdc04(self)
        else:
            if self.is_training:
