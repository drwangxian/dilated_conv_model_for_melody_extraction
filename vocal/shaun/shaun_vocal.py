...
adc04: 0.7570616677128643
mirex05: 0.8151631191806309
mdb: 0.808762166747679
mir1k: 0.6683833758369394
rwc: 0.7392825990096746
...
from vocal.shaun import acoustic_model_shaun as acoustic_model_module
...
class Config:
    ...
    def __init__(self,
        ...
        if mode == 'inference':
            ...
            assert inference_dataset in CONSTS.vocal_inference_allowed_datasets
        ...
        if is_inferencing:
            ...
            if self.inference_dataset == 'mdb':
                ...
                get_inf_track_ids_fn_dict = dict(
                    ...
                    mir1k=Config.get_mir1k_track_ids_fn,
                    rwc=Config.get_rwc_track_ids_fn
                )
                self.tvt_split_dict = Config.get_dataset_split_fn()
                self.tvt_split_dict['training'] = self.tvt_split_dict['training'][:1]
                self.tvt_split_dict['validation'] = self.tvt_split_dict['validation'][:1]
                self.tvt_split_dict['test'] = get_inf_track_ids_fn_dict[self.inference_dataset]()
                if self.debug_mode:
                    self.tvt_split_dict['test'] = self.tvt_split_dict['test'][:2]
        else:
            self.tvt_split_dict = Config.get_dataset_split_fn()
...
def main(
    ...
    def training_fn(global_step=None):
        ...
        for batch_idx in range(batches_per_epoch):
            ...
            metrics.update_states(ref_notes=batch['notes'][0], logits=logits, loss=loss_melody)
    ...
    if MODEL_DICT['config'].train_or_inference.inference is None:
        ...
        assert os.path.isabs(ckpt_prefix)
        ckpt_dir, ckpt_prefix = os.path.split(ckpt_prefix)
