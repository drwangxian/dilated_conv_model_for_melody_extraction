...
def load_samples_from_wav_file_fn(wav_file):
    ...
    sr = 44100
    samples, _sr = librosa.load(wav_file, sr=44100)
    assert _sr == sr
    _max = samples.max()
    _min = samples.min()
    assert _min >= -1
...
class NSGT:
    ...
    def _gen_gs_gds_ranges_fn(self):
        ...
        logging.debug('diagonal - minimum - {}'.format(diagonal.min()))
        # the theoretical minimum is : 2 / hop_size ** 2
        logging.debug('theoretical minimum - {}'.format(2. / hop_size ** 2))
        ...
        def _symmetry_chk_fn(diagonal):
            ...
            t = t_pos - t_neg
            t = np.sum(np.abs(t))
            logging.debug('diagonal difference -', t)
    ...
    def nsgt_of_wav_file_fn(self, wav_file_or_samples):
        ...
        if isinstance(wav_file_or_samples, np.ndarray):
            samples = wav_file_or_samples
        else:
            samples = load_samples_from_wav_file_fn(wav_file_or_samples)
