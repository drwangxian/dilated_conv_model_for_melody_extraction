"""
ablation study: global conv layer uses different dilations to cover the same receptive field
"""
...
class Config:
    ...
    partition = PARTITION
    assert partition in ((66, 15, 27), (67, 15, 26))

    def __init__(self):

        self.debug_mode = DEBUG
        Config.set_gpu_fn()
        assert DILATION in range(1, 5)
        ...
        if self.debug_mode:
            for split_name, split_tids in self.tvt_split_dict.items():
                self.tvt_split_dict[split_name] = split_tids[:3]
...
class AcousticModel:
    ...
    def __init__(self, config):
        ...
        acoustic_model = acoustic_model_module.create_acoustic_model_fn(config.dilation)
...
def main():
    ...
    aug_info = []
    aug_info.append('global dilation - {}'.format(MODEL_DICT['config'].dilation))
