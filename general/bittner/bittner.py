"""
bittner model
17-Nov-2021 Wednesday: support all of the datasets used by jiri
test performance
adc04: 73.77
mdb-s: 0.6264959126587383
mirex05: 0.7290757544857344
mdb: 0.6174899226260668
orchset: 0.42612831142299334
wjd: 0.7156831344824812
"""
...
import acoustic_model_bittner as acoustic_model_module
import json
import soundfile
import sqlite3
import mir_eval
from hcqt import hcqt_fn
...
class Config:
    ...
    def __init__(self):
        ...
        self.train_or_inference = Namespace(
            inference=os.path.join('..', '..', 'checkpoints/general/bittner', 'd0-17'),
            from_ckpt=None,
            ckpt_prefix=None
        )
        self.tb_dir = 'tb_inf'
        ...
        self.tvt_split_dict['test'] = self.get_orchset_track_ids_fn()
    ...
    def get_mdb_synth_test_track_ids_fn():
        ...
        intersection_track_id = 'MatthewEntwistle_DontYouEver'
        assert intersection_track_id not in mdb_synth_test_track_ids
...
class TFDataset:
    ...
    def hcqt_fn(wav_file):
        ...
        hop_size = TFDataset.spec_hop_size
        ...
        spec = hcqt_fn(wav_file)
        assert spec.shape == (num_frames, 360, 6)
    ...
    def gen_spec_fn(track_id):
        ...
        return TFDataset.hcqt_fn(mix_wav_file)
    ...
    def gen_label_fn(track_id):
        ...
        return dict(notes=notes, original=dict(times=times_labels[::2, 0], freqs=times_labels[::2, 1]))
...
class TFDatasetForAdc04(TFDataset):
    ...
    def gen_label_fn(track_id):
        ...
        freqs = times_labels[::2, 1]
        TFDataset.validity_check_of_ref_freqs_fn(freqs)
        notes = TFDataset.hz_to_midi_fn(freqs)

        return dict(notes=notes, original=dict(times=times_labels[::2, 0], freqs=freqs))
...
class TFDatasetForMirex05(TFDataset):
    ...
    def gen_label_fn(track_id):
        ...
        assert np.all(freqs_441 >= 0.)

        time_in_secs = times_labels[-1, 0]
        num_frames_256 = time_in_secs / (256. / 44100.)
        num_frames_256 = int(num_frames_256) + 1
        freqs_256 = np.empty([num_frames_256], dtype=np.float64)
        r_256_to_441 = 256. / 441.
        for frame_idx_256 in range(num_frames_256):

            frame_idx_441_lower = frame_idx_256 * 256 // 441
            frame_idx_441_upper = (frame_idx_256 * 256 + 440) // 441
            assert frame_idx_441_upper >= frame_idx_441_lower
            assert frame_idx_441_upper <= frame_idx_441_lower + 1

            theta_to_lower = frame_idx_256 * r_256_to_441 - frame_idx_441_lower
            assert 0. <= theta_to_lower <= 1.

            freq_lower = freqs_441[frame_idx_441_lower]
            freq_upper = freqs_441[frame_idx_441_upper]

            if freq_lower == 0. or freq_upper == 0.:
                freqs_256[frame_idx_256] = freq_lower if theta_to_lower < .5 else freq_upper
            else:
                freq = (1. - theta_to_lower) * freq_lower + theta_to_lower * freq_upper
                freqs_256[frame_idx_256] = freq
        ...
        return dict(notes=notes[::2], original=dict(times=times_labels[:, 0], freqs=times_labels[:, 1]))
...
class TFDatasetForOrchset(TFDataset):
    ...
    def gen_label_fn(track_id):
        ...
        num_frames_512 = 1 + ((num_frames - 1) * 441 + 511) // 512
        times_512 = np.arange(num_frames_512) * (512. / 44100.)
        assert times_512[-1] >= times_441[-1]
        freqs_512, _ = mir_eval.melody.resample_melody_series(
            times=times_441,
            frequencies=freqs_441,
            voicing=freqs_441 > .1,
            times_new=times_512
        )
        TFDataset.validity_check_of_ref_freqs_fn(freqs_512)
        notes = TFDataset.hz_to_midi_fn(freqs_512)
...
class TFDatasetForMDBSynthTest(TFDataset):
    ...
    def gen_label_fn(track_id):
        ...
        return dict(notes=notes, original=dict(times=times_labels[::4, 0], freqs=freqs))
...
class TFDatasetForWJazzd(TFDataset):
    ...
    def gen_label_fn(self, track_id):
        ...
        times = np.arange(num_frames) * (TFDataset.spec_hop_size / 44100.)
...
class MetricsInference(MetricsBase):
    ...
    def mir_eval_oa_fn(self, rec_idx, est_notes_with_voicing_info):
        ...
        est_times = np.arange(num_frames) * (hop_size / 44100.)
    ...
    def __init__(self, config, name):
        ...
        if inferencing:
            if 'test' not in name:
                ...
                self.tf_dataset = TFDatasetForOrchset(self)
        else:
            if self.is_training:
